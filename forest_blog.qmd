...
Here is a screenshot of the existing app to demonstrate the problem: 

```{=html}
<img src="blog_images/current.png" width = "100%"/>
```
...
In one part of the existing app, this was already causing a problem where three plots were being displayed side-by-side and even on a wide screen, the plot did not fit in the column. As a workaround, the plot was being rendered to a png file and then read back in to display. This fixed the problem of the edges being cut off, but there was actually a lot of white space around the plot which made it especially small. This workaround got me thinking though and I thought it would be much better if instead of rendering the plot to a png, it was rendered to an svg (scalable vector graphic), which, if inside an appropriate container can always scale to fit inside. SVGs are a great format and use code to produce graphics in an analagous way to how html can be used to produce text.

### svglite

Ideally, I was looking for a function that generates svg code, rather than writing to a file like `svg()` does to avoid writing and reading unneccessarily. After some searching I came across `svglite::xmlSVG()` which does exactly what I wanted. `svglite::stringSVG()` is very similar, but as we'll see later on, getting the output as an `{xml2}` object rather than a character has advantages when it comes to editing. The svg code output is shown below, followed by the resultant plot. The `paste(svg, collapse = "\n")` here and elsewhere is needed to convert the `{xml2}` object to a string.
...
In practice, we still need to calculate sensible values for width and height, but at least now we only have to do that once when we generate the plot and can rescale it however we like afterwards.
...
The width and height of the `viewBox` are also returned by the cropping function which enables rendering the plot to other file formats.
...
The downsides compared to before are that the plots aren't easily edited inside R and to be displayed in Rstudio, they need wrapping in `htmltools::HTML()`. Both of these would relatively easy to resolve though by adding a parameter to the function determine what type of output it produced - either the svg code, the svg wrapped in `htmltools::HTML()` or writing to the graphics device.

## End result

Here's the same interactive example after giving it the treatment.
